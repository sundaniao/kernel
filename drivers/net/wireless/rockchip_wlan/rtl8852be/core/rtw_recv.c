...
u8 rtw_init_recv_info(_adapter *adapter)
{
	...
	recvinfo->bcn_signal_strength = 88;
	recvinfo->bcn_rssi = 88 - 110;
...
int rtw_sta_rx_data_validate_hdr(_adapter *adapter, union recv_frame *rframe, struct sta_info **sta)
{
	...
	} else if (pmlmeinfo->state == WIFI_FW_NULL_STATE) {
		#if 1//def DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" info_state:0x%x\n"
			, FUNC_ADPT_ARG(adapter), pmlmeinfo->state);
		#endif
		goto exit;
	}
