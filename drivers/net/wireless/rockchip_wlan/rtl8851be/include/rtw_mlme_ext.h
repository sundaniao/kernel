...
#define	WIFI_FW_WAIT_BEACON		0x00008000
...
struct link_mlme_ext_priv {
	...
	u8 do_rfk_ap;
	/* Check if ever do rfk under ap mode. */
	/* If ever do, set to 1 */
#endif
...
u32 report_del_sta_event_ap(_adapter *padapter, u8 *mac_addr, u16 reason, bool enqueue, u8 locally_generated);
...
#endif /* CONFIG_P2P */
#ifdef CONFIG_CFG80211_SME_OFFLOAD
size_t rtw_build_assocreq_ies(struct _ADAPTER *padapter,
			      struct _ADAPTER_LINK *padapter_link,
			      u8 *frame,
			      struct pkt_attrib *pattrib,
			      u8 is_reassoc,
