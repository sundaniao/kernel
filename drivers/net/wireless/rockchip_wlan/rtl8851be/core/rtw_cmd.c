...
#define GEN_MLME_EXT_HANDLER(id, cmd, cb) [id] = { .name = #id, .cmd_hdl = cmd, .callback = cb},

static const struct rtw_cmd wlancmds[] = {
	GEN_MLME_EXT_HANDLER(CMD_JOINBSS, rtw_join_cmd_hdl, NULL)
...
const char *rtw_cmd_name(struct cmd_obj *pcmd)
{
...
u8 rtw_smeofld_wk_cmd(_adapter *adapter)
{
	struct cmd_obj *cmdobj;
	struct drvextra_cmd_parm *pdrvextra_cmd_parm;
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(adapter)->cmdpriv;
	...
	pdrvextra_cmd_parm =
		(struct drvextra_cmd_parm *)rtw_zmalloc(sizeof(struct drvextra_cmd_parm));
	if (pdrvextra_cmd_parm == NULL) {
		rtw_mfree((unsigned char *)cmdobj, sizeof(struct cmd_obj));
		res = _FAIL;
		goto exit;
	...
	pdrvextra_cmd_parm->ec_id = SMEOFLD_WK_CID;
	pdrvextra_cmd_parm->type = 0;
	pdrvextra_cmd_parm->size = 0;
	pdrvextra_cmd_parm->pbuf = NULL;
