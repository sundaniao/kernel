 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8851B_C_
#include "rtl8851b_hal.h"

static void _hal_trx_8851b_dump_rxcnt(struct hal_ppdu_sts *ppdu_sts)
{
...
static void _hal_dump_rxdesc(u8 *buf, struct rtw_r_meta_data *mdata)
{
 ...
 hal_handle_chaninfo_8851b(struct hal_info_t *hal)
 {
...
static enum rtw_hal_status
_hal_check_rxsc_rdy_8851b(struct hal_info_t *hal,
	struct rtw_phl_com_t *phl_com, u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	mdata->tid = GET_RX_AX_DESC_TID_8851B(desc);
	mdata->macid = GET_RX_AX_DESC_MACID_8851B(desc);
	mdata->seq = GET_RX_AX_DESC_SEQ_8851B(desc);
	mdata->mc = GET_RX_AX_DESC_MC_8851B(desc);
	mdata->bc = GET_RX_AX_DESC_BC_8851B(desc);
	mdata->htc = GET_RX_AX_DESC_HTC_8851B(desc);
	...
	if (mdata->ampdu &&
		...
		mdata->pwr_bit = GET_RX_AX_DESC_PWR_8851B(desc);
		mdata->q_null = GET_RX_AX_DESC_QNULL_8851B(desc);
		mdata->frame_type = GET_RX_AX_DESC_TYPE_8851B(desc);
 ...
 /**
 * SW Parsing Rx Desc - hal_parsing_rx_wd_8851b
 * description:
 ...
 */
static enum rtw_hal_status
_hal_parsing_rx_wd_8851b(struct hal_info_t *hal,
	...
	u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	if (   (mdata->rpkt_type == RX_8851B_DESC_PKT_T_WIFI)
	    || (mdata->rpkt_type == RX_8851B_DESC_PKT_T_PPDU_STATUS)) {
		...
		if (PHL_MACID_MAX_NUM != rxsc_entry->cached_rx_macid &&
			RX_8851B_DESC_PKT_T_WIFI == mdata->rpkt_type) {

			/* check and get rxsc cache */
			if (_hal_check_rxsc_rdy_8851b(hal, phl_com, desc, mdata) == RTW_HAL_STATUS_SUCCESS)
				return RTW_HAL_STATUS_SUCCESS;
		}
#endif
		...
		mdata->get_ch_info = GET_RX_AX_DESC_CH_INFO_8851B(desc);
		mdata->rx_statistics = GET_RX_AX_DESC_STATISTICS_8851B(desc);

		mdata->pattern_idx = GET_RX_AX_DESC_PATTERN_IDX_8851B(desc);
		mdata->target_idc = GET_RX_AX_DESC_TARGET_IDC_8851B(desc);
		mdata->chksum_ofld_en = GET_RX_AX_DESC_CHKSUM_OFFLOAD_8851B(desc);
		mdata->with_llc = GET_RX_AX_DESC_WITH_LLC_8851B(desc);

	if (mdata->long_rxd == 1) {
			mdata->frame_type = GET_RX_AX_DESC_TYPE_8851B(desc);
			mdata->mc = GET_RX_AX_DESC_MC_8851B(desc);
			mdata->bc = GET_RX_AX_DESC_BC_8851B(desc);
			mdata->more_data = GET_RX_AX_DESC_MD_8851B(desc);
			mdata->more_frag = GET_RX_AX_DESC_MF_8851B(desc);
			mdata->pwr_bit = GET_RX_AX_DESC_PWR_8851B(desc);
			mdata->qos = GET_RX_AX_DESC_QOS_8851B(desc);

			mdata->tid = GET_RX_AX_DESC_TID_8851B(desc);
			mdata->eosp = GET_RX_AX_DESC_EOSP_8851B(desc);
			mdata->htc = GET_RX_AX_DESC_HTC_8851B(desc);
			mdata->q_null = GET_RX_AX_DESC_QNULL_8851B(desc);

			mdata->seq = GET_RX_AX_DESC_SEQ_8851B(desc);
			mdata->frag_num = GET_RX_AX_DESC_FRAG_8851B(desc);

			mdata->sec_cam_idx = GET_RX_AX_DESC_CAM_IDX_8851B(desc);
			mdata->addr_cam = GET_RX_AX_DESC_ADDR_CAM_8851B(desc);

			mdata->addr_cam_vld = GET_RX_AX_DESC_CAM_VLD_8851B(desc);
			if (mdata->addr_cam_vld == 0)
				mdata->macid = 0xFF;
			else
				mdata->macid = GET_RX_AX_DESC_MACID_8851B(desc);

			mdata->rx_pl_id = GET_RX_AX_DESC_PL_ID_8851B(desc);

			mdata->addr_fwd_en = GET_RX_AX_DESC_FWD_EN_8851B(desc);
			mdata->rx_pl_match = GET_RX_AX_DESC_PL_MATCH_8851B(desc);

		_os_mem_cpy(hal_com->drv_priv,
			(void*)&mdata->mac_addr, (void*)(desc + 24), MAC_ALEN);
			mdata->sec_type = GET_RX_AX_DESC_SEC_TYPE_8851B(desc);
		}
	}
...
enum rtw_hal_status
hal_parsing_rx_wd_8851b(struct rtw_phl_com_t *phl_com,
				...
				struct rtw_r_meta_data *mdata)
{
	...
	do {
		...
		if (!halmac_rx) {
			hstatus = _hal_parsing_rx_wd_8851b(hal, phl_com, desc, mdata);
		} else {
		...
		desc_l = mdata->long_rxd ? RX_DESC_L_SIZE_8851B :
					   RX_DESC_S_SIZE_8851B;

		shift = (u8)(mdata->shift * 2 +
			     mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8851B +
			     desc_l);

		if ((1 == mdata->mac_info_vld) &&
		    (RX_8851B_DESC_PKT_T_PPDU_STATUS != mdata->rpkt_type))
			*pkt = desc + shift + RX_PPDU_MAC_INFO_SIZE_8851B;
		else
			*pkt = desc + shift;
...
static void
_hal_rx_wlanhdr_check_8851b(void *drvpriv, void *hdr, struct rtw_r_meta_data *mdata)
{
	/*Check Retry BIT*/
	u8 retry = 0;
	u16 seq_num = 0;
...
static void
_hal_rx_sts_8851b(struct hal_info_t *hal, struct rtw_r_meta_data *meta)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
enum rtw_hal_status
hal_handle_rx_buffer_8851b(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal,
				u8 *buf, u32 buf_len,
				struct rtw_phl_rx_pkt *phl_rx)
{
...
#endif
	hstatus = hal_parsing_rx_wd_8851b(phl_com, hal, buf,
...
#endif
		{
			_hal_rx_wlanhdr_check_8851b(drv, pkt->vir_addr, mdata);
			hal_rx_ppdu_sts_normal_data(phl_com, pkt->vir_addr, mdata);
		}
		_hal_rx_sts_8851b(hal, mdata);
	}
	break;
	case RX_8851B_DESC_PKT_T_TX_PD_RELEASE_HOST :
	{
		phl_rx->type = RTW_RX_TYPE_TX_WP_RELEASE_HOST;
	}
	break;
	case RX_8851B_DESC_PKT_T_PPDU_STATUS :
	{
		...
		PHL_TRACE(COMP_PHL_PSTS, _PHL_INFO_,
			  "==> RX_8851B_DESC_PKT_T_PPDU_STATUS :band %d ; ppdu_cnt  %d ; freerun_cnt %d \n",
		...
		if (ppdu_sts.rx_cnt_size != 0) {
			_hal_trx_8851b_dump_rxcnt(&ppdu_sts);
		}
		*/
		if (ppdu_sts.phy_st_size != 0) {
			if((mdata->ppdu_type == RX_8851B_DESC_PPDU_T_VHT_MU)||
			   (mdata->ppdu_type == RX_8851B_DESC_PPDU_T_HE_MU)||
			   (mdata->ppdu_type == RX_8851B_DESC_PPDU_T_HE_TB)) {
				is_su = 0;
#ifdef RTW_WKARD_MU_PPDU_STS_RX_RATE
	...
	break;
	case RX_8851B_DESC_PKT_T_DFS_RPT :
	{
	...
	case RX_8851B_DESC_PKT_T_CHANNEL_INFO :
	...
	case RX_8851B_DESC_PKT_T_F2P_TX_CMD_RPT :
	{
		/* DL MU Report ; UL OFDMA Trigger Report */
	}
	break;
	case RX_8851B_DESC_PKT_T_C2H :
	{
	...
	break;
	case RX_8851B_DESC_PKT_T_TX_RPT:
	{
