 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8851BE_C_
#include "../rtl8851b_hal.h"
#include "hal_trx_8851be.h"
 ...
 */
static u8 hal_query_txch_num_8851be(void)
{
	u8 ch_num = 0;

	ch_num = TX_DMA_CHANNEL_ENTRY_8851BE;
 ...
 */
static u8 hal_query_rxch_num_8851be(void)
{
	u8 ch_num = 0;

	ch_num = RX_DMA_CHANNEL_ENTRY_8851BE;

	return ch_num;
}
static u8 hal_qsel_to_tid_8851be(struct hal_info_t *hal, u8 qsel_id, u8 tid_indic)
{
	u8 tid = 0;
...
/**
 * Get target DMA channel's BD ram hw register of rtl8851b
 * @dma_ch: input target dma channel with the hw definition of rtl8851BE
 * return the BD ram hw register
 */
static u32 _hal_get_bd_ram_reg_8851be(u8 dma_ch)
{
	u32 reg = 0;

	switch (dma_ch) {
	case ACH0_QUEUE_IDX_8851BE:
		reg = R_AX_ACH0_BDRAM_CTRL;
		break;
	case ACH1_QUEUE_IDX_8851BE:
		reg = R_AX_ACH1_BDRAM_CTRL;
		break;
	case ACH2_QUEUE_IDX_8851BE:
		reg = R_AX_ACH2_BDRAM_CTRL;
		break;
	case ACH3_QUEUE_IDX_8851BE:
		reg = R_AX_ACH3_BDRAM_CTRL;
		break;
	case ACH4_QUEUE_IDX_8851BE:
		reg = R_AX_ACH4_BDRAM_CTRL;
		break;
	case ACH5_QUEUE_IDX_8851BE:
		reg = R_AX_ACH5_BDRAM_CTRL;
		break;
	case ACH6_QUEUE_IDX_8851BE:
		reg = R_AX_ACH6_BDRAM_CTRL;
		break;
	case ACH7_QUEUE_IDX_8851BE:
		reg = R_AX_ACH7_BDRAM_CTRL;
		break;
	case MGQ_B0_QUEUE_IDX_8851BE:
		reg = R_AX_CH8_BDRAM_CTRL;
		break;
	case HIQ_B0_QUEUE_IDX_8851BE:
		reg = R_AX_CH9_BDRAM_CTRL;
		break;
	case MGQ_B1_QUEUE_IDX_8851BE:
		reg = R_AX_CH10_BDRAM_CTRL;
		break;
	case HIQ_B1_QUEUE_IDX_8851BE:
		reg = R_AX_CH11_BDRAM_CTRL;
		break;
	case FWCMD_QUEUE_IDX_8851BE:
...
/**
 * Get target DMA channel's BD num hw register of rtl8851b
 * @dma_ch: input target dma channel with the hw definition of rtl8851BE
 * return the BD num hw register
 */
static u32 _hal_get_bd_num_reg_8851be(u8 dma_ch)
{
	u32 reg = 0;
	...
	case FWCMD_QUEUE_IDX_8851BE:
		...
		break;
	case RX_QUEUE_IDX_8851BE:
		reg = R_AX_RXQ_RXBD_NUM;
		break;
	case RP_QUEUE_IDX_8851BE:
...
/**
 * Get target DMA channel's BD Desc hw register of rtl8851b
 * @dma_ch: input target dma channel with the hw definition of rtl8851BE
 * return the BD Desc hw register
 */
static void _hal_get_bd_desc_reg_8851be(u8 dma_ch, u32 *addr_l, u32 *addr_h)
{
	u32 reg = 0;
...
/**
 * Get target DMA channel's BD Index hw register of rtl8851b
 * @dma_ch: input target dma channel with the hw definition of rtl8851BE
 * return the BD Index hw register
 */
static u32 _hal_get_bd_idx_reg_8851be(u8 dma_ch)
{
	u32 reg = 0;
 ...
 * to rtl8851BE hw tx dma channel
 ...
 *
 * returns the mapping hw dma channel defined by XXX_QUEUE_IDX_8851BE
 * if the input parameter is unknown value, returns ACH0_QUEUE_IDX_8851BE
 */
static u8 hal_mapping_hw_tx_chnl_8851be(struct hal_info_t *hal,
...
static void hal_query_txch_map_8851be(enum phl_band_idx band, void *ch_map)
{
...
static u8 hal_query_txch_hwband_8851be(u8 dma_ch)
{
	u8 band = 0;
	return band;
}
 ...
 */
static u16 hal_get_avail_txbd_8851be(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	...
	tx_dma_ch = ACH0_QUEUE_IDX_8851BE + ch_idx;

	reg = _hal_get_bd_idx_reg_8851be(tx_dma_ch);
	if (0xFFFF == reg) {
		...
		*hw_idx = (u16)((tmp32 >> 16) & 0x0FFF);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_get_avail_txbd_8851be => dma_ch %d, host_idx %d, "
...
static u16 hal_get_rxbd_num_8851be(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_num_8851be(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
...
static u16 hal_get_rxbuf_size_8851be(struct rtw_hal_com_t *hal_com, u8 ch_idx)
{
	u16 res = 0;
 ...
 * NOTE, input host_idx and hw_idx ptr shall NOT be NULL
 */
static u16 hal_get_avail_rxbd_8851be(struct rtw_hal_com_t *hal_com, u8 ch_idx,
				     u16 *host_idx, u16 *hw_idx)
{
	u16 avail_rxbd = 0;
	u32 tmp32 = 0, reg = 0;
	u8 rx_dma_ch = 0;
	u16 rxbd_num = hal_get_rxbd_num_8851be(hal_com, ch_idx);

	rx_dma_ch = RX_QUEUE_IDX_8851BE + ch_idx;

	reg = _hal_get_bd_idx_reg_8851be(rx_dma_ch);
	if (0xFFFF == reg) {
...
void _hal_fill_wp_seq_field_8851be(u8 *seq_info, u16 wp_seq)
{
	/* update sw wp seq */
...
void _hal_fill_wp_addr_info_8851be(struct rtw_hal_com_t *hal_com,
...
u8 _hal_get_tid_indic_8851be(u8 tid)
{
	u8 tid_indic = 0;
	switch (tid) {
		case RTW_PHL_RING_CAT_TID0:
...
_hal_txsc_update_wd_8851be(struct hal_info_t *hal,
						struct rtw_phl_pkt_req *req, u32 *wd_len)
{
	...
	dma_ch = hal_mapping_hw_tx_chnl_8851be(hal, mdata->macid,
						mdata->cat,
						mdata->band);
 ...
 */
static enum rtw_hal_status
hal_update_wd_8851be(struct hal_info_t *hal,
                     struct rtw_phl_pkt_req *req)
{
	...
	do {
...
#ifdef CONFIG_PHL_TXSC
		hstatus = _hal_txsc_update_wd_8851be(hal, req, &wd_len);
#else
		/* connect with halmac */
		hstatus = rtw_hal_mac_fill_txdesc(hal->mac,
		                                  tx_req,
		                                  ...
		                                  &wd_len);
#endif
		tid_indic = _hal_get_tid_indic_8851be(tx_req->mdata.tid);

		seq_ofst = wd_len;
		wp_seq = (1 == tid_indic) ?
				(req->wp_seq | WP_TID_INDIC_RESERVED_BIT) : req->wp_seq;
		_hal_fill_wp_seq_field_8851be(req->wd_page + seq_ofst, wp_seq);
		...
		for (i = 0; i < tx_req->mdata.addr_info_num; i++) {
			...
			_hal_fill_wp_addr_info_8851be(hal_com,
		...
		/* 8851BE length in txbd(wd_len) should be fix to 128 */
 ...
 */
static enum rtw_hal_status
hal_update_txbd_8851be(struct hal_info_t *hal,
		       ...
		       u8 ch_idx, u16 wd_num)
{
	...
	do {
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_update_txbd_8851be => ch_idx %d, wd_num %d\n",
		          ch_idx, wd_num);
 ...
 */
static enum rtw_hal_status
hal_trigger_txdma_8851be(struct hal_info_t *hal,
			struct tx_base_desc *txbd_ring, u8 ch_idx)
{
	...
	do {
		/* connect with halmac */
		tx_dma_ch = ACH0_QUEUE_IDX_8851BE + ch_idx;
		txbd_reg = _hal_get_bd_idx_reg_8851be(tx_dma_ch);
		if (0xFFFF == txbd_reg)
			break;
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			  "hal_trigger_txdma_8851be => dma_ch %d, host_idx %d.\n",
...
static enum rtw_hal_status hal_pltfm_tx_8851be(void *hal, struct rtw_h2c_pkt *h2c_pkt)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
#if 0
	...
	_os_spinlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8851BE].txbd_lock, _ps, NULL);
	hstatus = hal_update_txbd_8851BE(hal_info, txbd_ring, &wd_page,
							FWCMD_QUEUE_IDX_8851BE, 1);
	_os_spinunlock(hal_to_drvpriv(hal_info), &txbd_ring[FWCMD_QUEUE_IDX_8851BE].txbd_lock, _ps, NULL);

	/* enqueue busy queue */

	hstatus = hal_trigger_txdma_8851BE(hal_info, txbd_ring, FWCMD_QUEUE_IDX_8851BE);
#endif
	return hstatus;
}

u8 hal_get_fwcmd_queue_idx_8851be(void)
{
	return FWCMD_QUEUE_IDX_8851BE;
}

static u8 hal_check_rxrdy_8851be(struct rtw_phl_com_t *phl_com,
...
u16 hal_handle_rx_report_8851be(struct hal_info_t *hal, u8 *rp, u16 len,
				...
				u8 *macid, u8 *ac_queue, u8 *txsts)
{
	...
	do {
		...
		*wp_seq &= (WP_RESERVED_SEQ);
		tid = hal_qsel_to_tid_8851be(hal, qsel_value, tid_indic);
		*dma_ch = hal_mapping_hw_tx_chnl_8851be(hal, *macid, tid, band);
 ...
 */
static enum rtw_hal_status
hal_handle_ppdusts_8851be(void *hal, u8 *psbuf, u16 sz,
 ...
 **/
static enum rtw_hal_status
_hal_parsing_rx_wd_8851be(struct hal_info_t *hal, u8 *desc,
						struct rtw_r_meta_data *mdata)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;

	mdata->pktlen = GET_RX_AX_DESC_PKT_LEN_8851B(desc);
	mdata->shift = GET_RX_AX_DESC_SHIFT_8851B(desc);
	mdata->wl_hd_iv_len = GET_RX_AX_DESC_HDR_IV_L_8851B(desc);
	mdata->bb_sel = GET_RX_AX_DESC_BB_SEL_8851B(desc);
	mdata->mac_info_vld = GET_RX_AX_DESC_MAC_INFO_VLD_8851B(desc);
	mdata->rpkt_type = GET_RX_AX_DESC_RPKT_TYPE_8851B(desc);
	mdata->drv_info_size = GET_RX_AX_DESC_DRV_INFO_SIZE_8851B(desc);
	mdata->long_rxd = GET_RX_AX_DESC_LONG_RXD_8851B(desc);

	mdata->ppdu_type = GET_RX_AX_DESC_PPDU_TYPE_8851B(desc);
	mdata->ppdu_cnt = GET_RX_AX_DESC_PPDU_CNT_8851B(desc);
	mdata->sr_en = GET_RX_AX_DESC_SR_EN_8851B(desc);
	mdata->user_id = GET_RX_AX_DESC_USER_ID_8851B(desc);
	mdata->rx_rate = GET_RX_AX_DESC_RX_DATARATE_8851B(desc);
	mdata->rx_gi_ltf = GET_RX_AX_DESC_RX_GI_LTF_8851B(desc);
	mdata->non_srg_ppdu = GET_RX_AX_DESC_NON_SRG_PPDU_8851B(desc);
	mdata->inter_ppdu = GET_RX_AX_DESC_INTER_PPDU_8851B(desc);
	mdata->bw = GET_RX_AX_DESC_BW_8851B(desc);

	mdata->freerun_cnt = GET_RX_AX_DESC_FREERUN_CNT_8851B(desc);

	mdata->a1_match = GET_RX_AX_DESC_A1_MATCH_8851B(desc);
	mdata->sw_dec = GET_RX_AX_DESC_SW_DEC_8851B(desc);
	mdata->hw_dec = GET_RX_AX_DESC_HW_DEC_8851B(desc);
	mdata->ampdu = GET_RX_AX_DESC_AMPDU_8851B(desc);
	mdata->ampdu_end_pkt = GET_RX_AX_DESC_AMPDU_EDN_PKT_8851B(desc);
	mdata->amsdu = GET_RX_AX_DESC_AMSDU_8851B(desc);
	mdata->amsdu_cut = GET_RX_AX_DESC_AMSDU_CUT_8851B(desc);
	mdata->last_msdu = GET_RX_AX_DESC_LAST_MSDU_8851B(desc);
	mdata->bypass = GET_RX_AX_DESC_BYPASS_8851B(desc);
	mdata->crc32 = GET_RX_AX_DESC_CRC32_8851B(desc);
	mdata->icverr = GET_RX_AX_DESC_ICVERR_8851B(desc);
	mdata->magic_wake = GET_RX_AX_DESC_MAGIC_WAKE_8851B(desc);
	mdata->unicast_wake = GET_RX_AX_DESC_UNICAST_WAKE_8851B(desc);
	mdata->pattern_wake = GET_RX_AX_DESC_PATTERN_WAKE_8851B(desc);

	if (mdata->long_rxd==1) {
		mdata->macid = GET_RX_AX_DESC_MACID_8851B(desc);
	}
...
static u8 hal_handle_rxbd_info_8851be(struct hal_info_t *hal,
 ...
 */
static enum rtw_hal_status
hal_update_rxbd_8851be(struct hal_info_t *hal, struct rx_base_desc *rxbd,
		       struct rtw_rx_buf *rx_buf, u8 ch_idx)
{
	...
	do {
		...
		/* Early check to ensure rxbd_num is valid */
		if (0 == rxbd_num) {
			hstatus = RTW_HAL_STATUS_FAILURE;
			break;
		}
 ...
 */
static enum rtw_hal_status
hal_notify_rxdone_8851be(struct hal_info_t *hal,
		struct rx_base_desc *rxbd, u8 ch, u16 rxcnt)
{
	...
	do {
		rx_dma_ch = RX_QUEUE_IDX_8851BE + ch;
		reg = _hal_get_bd_idx_reg_8851be(rx_dma_ch);
		/* connect with halmac */
		if (0xFFFF == reg)
			break;
...
struct bd_ram bdram_table_8851b[] = {
		/* ACH0_QUEUE_IDX_8851BE */ {0, 25, 4},
		/* ACH1_QUEUE_IDX_8851BE */ {25, 25, 4},
		/* ACH2_QUEUE_IDX_8851BE */ {50, 25, 4},
		/* ACH3_QUEUE_IDX_8851BE */ {75, 25, 4},
		/* ACH4_QUEUE_IDX_8851BE */ {100, 25, 4},
		/* ACH5_QUEUE_IDX_8851BE */ {125, 25, 4},
		/* ACH6_QUEUE_IDX_8851BE */ {150, 25, 4},
		/* ACH7_QUEUE_IDX_8851BE */ {175, 25, 4},
		/* MGQ_B0_QUEUE_IDX_8851BE */ {185, 10, 4},
		/* HIQ_B0_QUEUE_IDX_8851BE */ {195, 10, 4},
		/* MGQ_B1_QUEUE_IDX_8851BE */ {205, 10, 4},
		/* HIQ_B1_QUEUE_IDX_8851BE */ {215, 10, 4},
		/* FWCMD_QUEUE_IDX_8851BE */ {225, 10, 4}
};

static void _hal_tx_init_bd_ram_8851be(struct hal_info_t *hal)
{
	u32 reg = 0;
	u32 value = 0;
	u8 i = 0;

	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8851BE; i++) {
		/*if (FWCMD_QUEUE_IDX_8851BE == i)
			continue;*/
		value = 0;
		value = bdram_table_8851b[i].sidx +
			((bdram_table_8851b[i].max << 8) & 0xFF00) +
			((bdram_table_8851b[i].min << 16) & 0xFF0000);

		reg = _hal_get_bd_ram_reg_8851be(i);
		if (0 != reg)
...
static void _hal_trx_init_bd_num_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u16 txbd_num = hal_com->bus_cap.txbd_num;
	u16 rxbd_num = 0;
	...
	for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8851BE; i++) {
		...
		reg = _hal_get_bd_num_reg_8851be(i);
		if (0 != reg)
	...
	for (i = RX_QUEUE_IDX_8851BE;
		 i < RX_QUEUE_IDX_8851BE + RX_DMA_CHANNEL_ENTRY_8851BE; i++) {
		 /*if (FWCMD_QUEUE_IDX_8851BE == i)
			 continue;*/

		 ch_idx = (u8)(i - RX_QUEUE_IDX_8851BE);
		 rxbd_num = hal_get_rxbd_num_8851be(hal_com, ch_idx);
...
static void _hal_trx_init_bd_8851be(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	...
	if (NULL != txbd_buf && NULL != rxbd_buf) {
		...
		for (i = 0; i < TX_DMA_CHANNEL_ENTRY_8851BE; i++) {
			...
			_hal_get_bd_desc_reg_8851be(i, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
		...
		for (i = 0; i < RX_DMA_CHANNEL_ENTRY_8851BE; i++) {
			 rxch_idx = i + RX_QUEUE_IDX_8851BE;
			_hal_get_bd_desc_reg_8851be(rxch_idx, &reg_addr_l, &reg_addr_h);
			if (0 != reg_addr_l)
...
/**
 * the function will deinitializing 8851BE specific data and hw configuration
 */
static void hal_trx_deinit_8851be(struct hal_info_t *hal)
{
	/*struct rtw_hal_com_t *hal_com = hal->hal_com;*/
}

/**
 * the function will initializing 8851BE specific data and hw configuration
 */
static enum rtw_hal_status hal_trx_init_8851be(struct hal_info_t *hal, u8 *txbd_buf, u8 *rxbd_buf)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
...
void hal_trx_ops_init_8851be(void)
{
	ops.init = hal_trx_init_8851be;
	ops.deinit = hal_trx_deinit_8851be;
	ops.query_tx_res = hal_get_avail_txbd_8851be;
	ops.query_rx_res = hal_get_avail_rxbd_8851be;
	ops.get_rxbd_num = hal_get_rxbd_num_8851be;
	ops.get_rxbuf_num = hal_get_rxbuf_num_8851be;
	ops.get_rxbuf_size = hal_get_rxbuf_size_8851be;
	ops.map_hw_tx_chnl = hal_mapping_hw_tx_chnl_8851be;
	ops.qsel_to_tid = hal_qsel_to_tid_8851be;
	ops.query_txch_hwband = hal_query_txch_hwband_8851be;
	ops.query_txch_map = hal_query_txch_map_8851be;
	ops.query_txch_num = hal_query_txch_num_8851be;
	ops.query_rxch_num = hal_query_rxch_num_8851be;
	ops.update_wd = hal_update_wd_8851be;
	ops.update_txbd = hal_update_txbd_8851be;
	ops.tx_start = hal_trigger_txdma_8851be;
	ops.check_rxrdy = hal_check_rxrdy_8851be;
	ops.handle_rxbd_info = hal_handle_rxbd_info_8851be;
	ops.handle_rx_buffer = hal_handle_rx_buffer_8851b;
	ops.update_rxbd = hal_update_rxbd_8851be;
	ops.notify_rxdone = hal_notify_rxdone_8851be;
	ops.handle_wp_rpt = hal_handle_rx_report_8851be;
	ops.get_fwcmd_queue_idx = hal_get_fwcmd_queue_idx_8851be;
}



u32 hal_hook_trx_ops_8851be(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;

	if (NULL != hal_info) {
		hal_trx_ops_init_8851be();
