...
enum rtw_hal_status
hal_get_pcicfg_8851be(struct hal_info_t *hal_info,
...
static enum rtw_hal_status _hal_aspm_disable_8851be(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_sw_init_state_8851be(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_set_pcie_8851be(struct hal_info_t *hal_info)
{
...
static void _hal_tx_ch_config_8851be(struct hal_info_t *hal)
{
...
static void _hal_pre_init_8851be(struct rtw_phl_com_t *phl_com,
				 struct hal_info_t *hal_info,
				 struct hal_init_info_t *init_51be)
{
	struct mac_ax_trx_info *trx_info = &init_51be->trx_info;
	struct mac_ax_intf_info *intf_info = &init_51be->intf_info;
	...
	_hal_tx_ch_config_8851be(hal_info);
	...
	/* others */
	init_51be->ic_name = "rtl8851be";
}

void init_hal_spec_8851be(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	init_hal_spec_8851b(phl_com, hal);
...
enum rtw_hal_status hal_get_efuse_8851be(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_51be;
	_os_mem_set(hal_to_drvpriv(hal_info), &init_51be, 0, sizeof(init_51be));
	_hal_pre_init_8851be(phl_com, hal_info, &init_51be);

	return hal_get_efuse_8851b(phl_com, hal_info, &init_51be);
}

enum rtw_hal_status hal_set_pcicfg_8851be(struct hal_info_t *hal_info)
{
	struct mac_ax_pcie_cfgspc_param pcicfg;
	enum rtw_hal_status hsts = RTW_HAL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
...
enum rtw_hal_status hal_fast_start_8851be(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_51be;
	...
	hal_status = hal_fast_start_8851b(phl_com, hal_info, &init_51be);
	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_fast_start_8851b: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = hal_set_pcicfg_8851be(hal_info);
	if (RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("hal_set_pcicfg_8851be: status = %u\n",hal_status);
		return hal_status;
	}
...
enum rtw_hal_status hal_fast_stop_8851be(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	return hal_fast_stop_8851b(phl_com, hal_info);
}

enum rtw_hal_status hal_init_8851be(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
...
void hal_deinit_8851be(struct rtw_phl_com_t *phl_com,
		       struct hal_info_t *hal_info)
{
	u8 txch_num = 0;
...
enum rtw_hal_status hal_start_8851be(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_51be;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	hal_status = hal_start_8851b(phl_com, hal_info, &init_51be);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_init_8851b: status = %u\n",hal_status);
		return hal_status;
	}
	...
	hal_status = _hal_ltr_set_pcie_8851be(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_set_pcie_8851be: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_ltr_sw_init_state_8851be(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_sw_init_state_8851be: status = %u\n",hal_status);
		return hal_status;
	}
...
enum rtw_hal_status hal_stop_8851be(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	if (TEST_STATUS_FLAG(phl_com->dev_state, RTW_DEV_SHUTTING_DOWN)) {
		hal_status = _hal_aspm_disable_8851be(hal);
		if (RTW_HAL_STATUS_SUCCESS != hal_status)
			PHL_ERR("%s: _hal_aspm_disable_8851be status = %u\n",
				__func__, hal_status);
	}

	hal_status = hal_stop_8851b(phl_com, hal);
	return hal_status;
}
...
enum rtw_hal_status
hal_wow_init_8851be(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
							struct rtw_phl_stainfo_t *sta)
{
	...
	*/
	...
	hal_status = hal_wow_init_8851b(phl_com, hal_info, sta, &init_51be);
...
enum rtw_hal_status
hal_wow_deinit_8851be(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
								struct rtw_phl_stainfo_t *sta)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_51be;
	...
	*/
	...
	hal_status = hal_wow_deinit_8851b(phl_com, hal_info, sta, &init_51be);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_wow_deinit_8851be: status = %u\n", hal_status);
		return hal_status;
	}

	if (RTW_HAL_STATUS_SUCCESS != hal_set_pcicfg_8851be(hal_info))
		PHL_ERR("hal_set_pcicfg_8851be: status = %u\n", hal_status);
...
u32 hal_hci_cfg_8851be(struct rtw_phl_com_t *phl_com,
...
void hal_init_default_value_8851be(struct hal_info_t *hal)
{
	init_default_value_8851b(hal);

	hal_init_int_default_value_8851be(hal, INT_SET_OPT_HAL_INIT);
}

void hal_init_int_default_value_8851be(struct hal_info_t *hal, enum rtw_hal_int_set_opt opt)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	...
	hal_com->int_mask_default[0] = (u32)(
		...
		B_AX_HS0ISR_IND_INT_EN_WKARND | /* FW C2H*/
...
void hal_enable_int_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_disable_int_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_disable_int_isr_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
bool hal_recognize_int_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_int_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_int_mask_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_restore_int_8851be(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_bcn_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_rx_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	static const u32 rx_handle_irq = B_AX_RXDMA_INT_EN | B_AX_RDU_INT_EN;
	static const u32 rx_handle_irq_imr = B_AX_RPQDMA_INT_EN |
	...
	hal_com->int_mask[0] &= ~rx_handle_irq_imr;
#ifndef CONFIG_SYNC_INTERRUPT
	hal_write32(hal_com, R_AX_PCIE_HIMR00, hal_com->int_mask[0]);
#endif /* CONFIG_SYNC_INTERRUPT */
#ifdef PHL_RXSC_ISR
...
static u32 hal_rp_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	static const u32 rp_handle_irq = B_AX_RPQDMA_INT_EN | B_AX_RPQBD_FULL_INT_EN;
	static const u32 rp_handle_irq_imr = B_AX_RPQDMA_INT_EN |
	                                     B_AX_RPQBD_FULL_INT_EN |
	                                     B_AX_RXDMA_INT_EN |
	                                     B_AX_RDU_INT_EN;
	...
	PHL_TRACE(COMP_PHL_DBG, _PHL_DEBUG_, "RX IRQ B4 : %08X (%08X)\n", handled0, hal_com->int_array[0]);
	/* Disable RX interrupts, RX tasklet will enable them after processed RX */
	hal_com->int_mask[0] &= ~rp_handle_irq_imr;
#ifndef CONFIG_SYNC_INTERRUPT
	hal_write32(hal_com, R_AX_PCIE_HIMR00, hal_com->int_mask[0]);
#endif /* CONFIG_SYNC_INTERRUPT */
#ifdef PHL_RXSC_ISR
...
static u32 hal_tx_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u32 event0 = 0x0;
...
static u32 hal_cmd_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_halt_c2h_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
#if 1
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_watchdog_timer_handler_8851be(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
#if 1
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
u32 hal_int_hdler_8851be(struct hal_info_t *hal)
{
	...
	/* <1> beacon related */
	int_hdler_msk |= (hal_bcn_handler_8851be(hal, handled)<<0);

	/* <2> Rx related */
	int_hdler_msk |= (hal_rx_handler_8851be(hal, handled)<<1);

	/* <3> Tx related */
	int_hdler_msk |= (hal_tx_handler_8851be(hal, handled)<<2);

	/* <4> Cmd related */
	int_hdler_msk |= (hal_cmd_handler_8851be(hal, handled)<<3);

	/*Start General interrupt type*/
	/* <5> Halt C2H related */
	int_hdler_msk |= (hal_halt_c2h_handler_8851be(hal, generalhandled)<<4);

	/* <6> watchdog timer related */
	int_hdler_msk |= (hal_watchdog_timer_handler_8851be(hal, generalhandled)<<5);

	/* bit 7 : rsvd for gt3 interrupt*/

	/* bit 8 : rx rp queue related */
	int_hdler_msk |= (hal_rp_handler_8851be(hal, handled) << 8);
...
void hal_rx_int_restore_8851be(struct hal_info_t *hal)
{
...
enum rtw_hal_status
hal_rx_rpq_int_check_8851be(u8 dma_ch, u32 hal_int_array)
{
...
enum rtw_hal_status
hal_mp_init_8851be(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_51be;
	...
	hal_status = hal_mp_init_8851b(phl_com, hal_info, &init_51be);
...
enum rtw_hal_status
hal_mp_deinit_8851be(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_51be;
	...
	hal_status = hal_mp_deinit_8851b(phl_com, hal_info, &init_51be);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_mp_deinit_8851be: status = %u\n", hal_status);
		return hal_status;
	}
...
bool
hal_mp_path_chk_8851be(struct rtw_phl_com_t *phl_com, u8 ant_tx, u8 cur_phy)
{
	if (phl_com->phy_cap[cur_phy].txss == 1 && ant_tx != RF_PATH_A)
		return false;
	else
		return true;
}
