					...
					struct hal_info_t *hal)
{
	...
	hal_com->phy_hw_cap[0].rx_path_num = 1;
	hal_com->phy_hw_cap[0].tx_path_num = 1;
	hal_com->phy_hw_cap[1].rx_num = 1;
	hal_com->phy_hw_cap[1].tx_num = 1;
	hal_com->phy_hw_cap[1].rx_path_num = 1;
	hal_com->phy_hw_cap[1].tx_path_num = 1;
	...
	hal_com->dev_hw_cap.hw_stype_cap = EFUSE_HW_STYPE_NONE_8851B;
	...
	hal_com->dev_hw_cap.rfk_fcs_sup = false;
...
u32 _hal_cfg_rom_fw_8851b(enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info,
			  char *ic_name)
{
	...
	_os_snprintf(fw_info->rom_path, MAX_PATH_LEN, "%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "rtl8851bfw_rom.bin");
...
u32 _hal_cfg_intnal_fw_8851b(struct rtw_phl_com_t *phl_com,enum rtw_fw_type fw_type,struct rtw_fw_info_t *fw_info)
{
	/* any related to fw from header can be defined here */
	return RTW_HAL_STATUS_SUCCESS;
}

static u32 _hal_read_fw_8851b(enum rtw_fw_type fw_type,
	u8 *tgt_buf, u32 *tgt_buf_size, char *ic_name)
{
	...
	_os_snprintf(path, MAX_PATH_LEN, "%s%s%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "RTL8851Bfw", filename_postfix, ".bin");
...
static u32 _hal_cfg_extnal_fw_8851b(struct rtw_phl_com_t *phl_com,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	...
	switch (fw_type) {
	case RTW_FW_NIC:
		_hal_read_fw_8851b(RTW_FW_NIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		/* preload wowlan fw */
		_hal_read_fw_8851b(RTW_FW_WOWLAN, fw_info->wow_buf,
	...
	case RTW_FW_WOWLAN:
		...
		break;
	case RTW_FW_SPIC:
		_hal_read_fw_8851b(RTW_FW_SPIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	case RTW_FW_AP:
		_hal_read_fw_8851b(RTW_FW_AP, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	...
	if (tgt_buf && (0 != tgt_buf_size)) {
		...
		fw_info->ram_size = 0;
		return _hal_read_fw_8851b(fw_type, fw_info->ram_buff,
				     ...
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (RTW_HAL_STATUS_SUCCESS !=_hal_cfg_rom_fw_8851b(fw_type, fw_info,
			ic_name))
			goto init_fw_fail;
	} else {
		if(fw_info->rom_buff) {
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8851b(phl_com, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_intnal_fw_8851b(phl_com, fw_type,
			fw_info))
			goto init_fw_fail;
	} else {
		goto init_fw_fail;
...
enum rf_path
hal_get_path_from_ant_num_8851b(u8 antnum){
	enum rf_path ret = RF_PATH_A;
...
enum rtw_hal_status hal_fast_start_8851b(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
					struct hal_init_info_t *init_info)
{
...
enum rtw_hal_status hal_fast_stop_8851b(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	struct hal_init_info_t *init_info)
{
	...
	psts_cfg.filter = HAL_PPDU_HAS_CRC_OK | HAL_PPDU_HAS_A1M;
...
enum rtw_hal_status hal_stop_8851b(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
#ifdef CONFIG_WOWLAN
enum rtw_hal_status
hal_wow_init_8851b(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
...
enum rtw_hal_status
hal_wow_deinit_8851b(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
                     struct rtw_phl_stainfo_t *sta,
                     struct hal_init_info_t *init_info)
{
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
...
#ifdef RTW_PHL_BCN /* fill 8851b bcn ops */
