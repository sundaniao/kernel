	...
	u8 loss_only)
{
	...
	do {
		...
		case 4:
			if((rf->phl_com->dev_cap.rfe_type == 2) && (dpk->bp[path][kidx].band != BAND_ON_24G))
				_dpk_kip_set_txagc_8851b(rf, phy, path, tmp_dbm - tmp_gl_idx - 2, true);
	...
	u8 kidx)
{
	...
	if ((rf->phl_com->dev_cap.rfe_type == 2) && (dpk->bp[path][kidx].band != BAND_ON_24G)) {
		dpk->bp[path][kidx].txagc_dpk = dpk->bp[path][kidx].txagc_dpk - 2;
