...
u8 halrf_get_thermal_8851b(struct rf_info *rf, enum rf_path rf_path)
{
	halrf_wrf(rf, RF_PATH_A, 0x42, BIT(19), 0x1);
	halrf_wrf(rf, RF_PATH_A, 0x42, BIT(19), 0x0);
	halrf_wrf(rf, RF_PATH_A, 0x42, BIT(19), 0x1);

	halrf_delay_us(rf, 200);

	return (u8)halrf_rrf(rf, RF_PATH_A, 0x42, 0x0007e);
}

u32 halrf_mac_get_pwr_reg_8851b(struct rf_info *rf, enum phl_phy_idx phy,
	u32 addr, u32 mask)
{
...
u32 halrf_mac_set_pwr_reg_8851b(struct rf_info *rf, enum phl_phy_idx phy,
...
void halrf_wlan_tx_power_control_8851b(struct rf_info *rf,
	...
	u32 tx_power_val, bool enable)
{
	...
	if (enable == false) {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			...
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd200, 0x3ff, 0x0);
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			/*GNT_BT control Disable*/
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd220, BIT(1), 0x0);
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd220, 0xff8, 0x0);
		}
	} else {
		if (pwr_ctrl_idx == ALL_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd200, 0x3ff,
				((tx_power_val & 0x1ff) | BIT(9)));
		} else if (pwr_ctrl_idx == GNT_TIME_CTRL) {
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd220, BIT(1), 0x1);
			halrf_mac_set_pwr_reg_8851b(rf, phy_tmp, 0xd220, 0xff8, tx_power_val & 0x1ff);
		} else {
...
bool halrf_wl_tx_power_control_8851b(struct rf_info *rf, u32 tx_power_val)
{
	...
	if (pwr->coex_pwr_ctl_enable == false && pwr->dpk_pwr_ctl_enable == false) {
		/*all-time control Disable*/
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd200, 0x3ff, 0x0);
		...
		/*all-time control*/
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd200, 0x3ff, ((tmp_pwr & 0x1ff) | BIT(9)));
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd200) fail, error code=%d\n",
				__func__, result);
			return false;
	...
	if (gnt_bt_control == 0xffff) {
		...
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd220, BIT(1), 0x0);
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd220, 0xff8, 0x0);
		if (result) {
		...
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd220, BIT(1), 0x1);
		result = halrf_mac_set_pwr_reg_8851b(rf, phy, 0xd220, 0xff8, gnt_bt_control & 0x1ff);
		if (result) {
			RF_WARNING("=======>%s Set MAC(0xd220) fail, error code=%d\n",
				__func__, result);
			return false;
...
s8 halrf_get_ther_protected_threshold_8851b(struct rf_info *rf)
{
	u8 tmp;
	u8 therml_max = 0x32;
...
s8 halrf_xtal_tracking_offset_8851b(struct rf_info *rf,
...
void halrf_set_gpio_8851b(struct rf_info *rf, enum phl_phy_idx phy)
{
	...
	/*dual antenna with diversity*/
	if (rfe_type < 50 && (rfe_type % 3) == 2) {
		/*Set BB GPIO Setting*/
		/*PathA=0x1*/
		halrf_gpio_setting(rf, 16, 0x1, false, HALRF_ANTSEL_0);
		halrf_gpio_setting(rf, 17, 0x1, true, HALRF_ANTSEL_0);

		/*Set MAC GPIO Setting*/
		halrf_set_gpio_func(rf, RTW_MAC_GPIO_WL_RFE_CTRL, 16);
		halrf_set_gpio_func(rf, RTW_MAC_GPIO_WL_RFE_CTRL, 17);

		rtw_hal_mac_set_eecs_pull(rf->hal_com, 0x2);	/*MAC_AX_GPIO_PULL_LOW*/
		rtw_hal_mac_set_eesk_pull(rf->hal_com, 0x1);	/*MAC_AX_GPIO_PULL_HIGH*/
	} 
}

bool halrf_chlk_reload_check_8851b(struct rf_info *rf, enum phl_phy_idx phy) 
{
