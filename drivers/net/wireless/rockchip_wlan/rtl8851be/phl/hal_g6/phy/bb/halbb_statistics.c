...
u32 halbb_get_fw_c2h_statistics(struct bb_info *bb_0, u16 len, u8 *c2h)
{
	...
	if (c2h_rpt->center_ch == 0 && bb->hal_com->band[c2h_rpt->phy_idx].cur_chandef.center_ch == 0) {
		BB_WARNING("[%s] phy=%d, fc{fw, drv}={%d, %d}\n", __func__,
				c2h_rpt->phy_idx, c2h_rpt->center_ch,
				bb->hal_com->band[c2h_rpt->phy_idx].cur_chandef.center_ch);
		return _FAIL;
	}
