...
//2 MP test API
u32 do_xtal_test(struct mac_ax_adapter *adapter)
{
	struct mac_ax_ops *ops = adapter_to_mac_ops(adapter);
	u8 val = 0;
	u32 ret = MACSUCCESS;

	// XTAL_SI 0x3[3:0] = 0xB, set A-die AON step down
	ret = ops->write_xtal_si(adapter, R_XTAL_SI_AON_LDO, A_DIE_AON_VOL,
				 B_A_DIE_AON_VOL_MSK);
	if (ret) {
	...
	// XTAL_SI 0x24[6:4] = 0x2, set XTAL LDO step up
	ret = ops->write_xtal_si(adapter, R_XTAL_SI_XTAL_LDO, A_DIE_XTAL_LPS_VOL,
				 B_A_DIE_XTAL_LPS_VOL_MSK);
	if (ret) {
		PLTFM_MSG_ERR("Write XTAL_SI fail!\n");
		return ret;
	}

	// XTAL_SI 0x0[3:0] = 0x8, switch to XTAL LPS mode
	ret = ops->write_xtal_si(adapter, R_XTAL_SI_XTAL_MODE, XTLA_LPS_MODE,
				 B_XTAL_MODE_MSK);
	if (ret) {
		PLTFM_MSG_ERR("Write XTAL_SI fail!\n");
		return ret;
	...
	// XTAL_SI 0x0[3:0] = 0x3, swtich to XTAL normal mode
	ret = ops->write_xtal_si(adapter, R_XTAL_SI_XTAL_MODE, XTLA_NOR_MODE,
				 B_XTAL_MODE_MSK);
	if (ret) {
		PLTFM_MSG_ERR("Write XTAL_SI fail!\n");
		return ret;
	...
	// check A die reset or not
	ret = ops->read_xtal_si(adapter, R_XTAL_SI_XTAL_MODE, &val);
	if (ret) {
...
#if MAC_AX_8852A_SUPPORT || MAC_AX_8852B_SUPPORT || MAC_AX_8192XB_SUPPORT || \
