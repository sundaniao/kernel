		     ...
		     char *output, u32 out_len, u32 *used)
{
	...
	u32 reg1 = 0, reg2 = 0, b0_backup_val32 = 0, i = 0, ret = 0;
	u32 ch_used_pg_previous[8] = {0};
	u32 ch_used_pg_current[8] = {0};
				...
				*total_check_number += 1;
				if (ch_used_pg_current[i] < ch_used_pg_previous[i]) {
					MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
						    "func: %s, Reg: R_BE_CCA_CFG_0 => %x[4:0],"\
						    "Disable [Band0] CCA can consume [CH%d] Page, "\
						    "please check [Band0] CCA configuration\n",\
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852A) ||
		...
		for (link->wmm = 0; link->wmm <= 3; link->wmm++) {
			for (link->ac = 0; link->ac <= 3; link->ac++) {
				...
				if (link->link_len != 0) {
					if (link->link_head == (adapter->hw_info->macid_num - 1)) {
					...
					if (link->link_tail == (adapter->hw_info->macid_num - 1)) {
						MAC_DBG_MSG(out_len, *used, output +
							    *used, out_len - *used,
							    "WMM[%d], AC[%d]"\
					...
					*total_check_number += 2;
					...
					if (link->link_head != (adapter->hw_info->macid_num - 1)) {
						MAC_DBG_MSG(out_len, *used, output +
							    *used, out_len - *used,
							    "WMM[%d], AC[%d]"\
							    "Head of STA_SCH_LIST should"\
							    "equal 0x%x."\
							    "The length of List is 0"\
							    "Please Check STA_SCH List\n",\
							    link->wmm, link->ac,
							    adapter->hw_info->macid_num - 1);
						cnt += 1;
					}
					if (link->link_tail != (adapter->hw_info->macid_num - 1)) {
						MAC_DBG_MSG(out_len, *used, output +
		    ...
		    char *output, u32 out_len, u32 *used)
{
	...
	*total_check_number = 0;
	...
	cnt += diagnosis_FW_system(adapter, total_check_number, output, out_len, used);
	if (cnt - cnt_tmp > 0)
		MAC_DBG_MSG(out_len, *used, output + *used, out_len - *used,
