		     ...
		     struct mac_ax_bcn_info *info)
{
	...
	for(i = 0; i < MAX_NUM_HW_TWT; i++) {
		idx = info->twt_ofld[i].id;
		twt_ofst[idx] = (u16)((u8)(info->twt_ofld[idx].en) | (info->twt_ofld[idx].ofst << 1));
	}
	...
	sub_content->dword6 =
		cpu_to_le32(SET_WORD(twt_ofst[0],
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST0) |
			    SET_WORD(twt_ofst[1],
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST1));

	sub_content->dword7 =
		cpu_to_le32(SET_WORD(twt_ofst[2],
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST2) |
			    SET_WORD(twt_ofst[3],
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST3));

	sub_content->dword8 =
		cpu_to_le32(SET_WORD(info->twt_ofld[0].id,
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST0_ID) |
			    SET_WORD(info->twt_ofld[1].id,
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST1_ID) |
			    SET_WORD(info->twt_ofld[2].id,
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST2_ID) |
			    SET_WORD(info->twt_ofld[3].id,
				     FWCMD_H2C_BCN_UPD_V1_TWT_OFST3_ID));
