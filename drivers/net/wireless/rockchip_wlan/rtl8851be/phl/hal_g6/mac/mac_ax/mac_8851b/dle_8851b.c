...
static struct mac_ax_dle_dfi_info dle_dfi_ple_quemgn_qlnktbl = {
	0,
	(PLE_QLINK_TBL_NUM(8851B) - 1),
	1
};

static struct mac_ax_dle_dfi_info dle_dfi_ple_quemgn_qempty = {
	0,
	(PLE_QEMPTY_NUM(8851B) - 1),
	1
};

u32 dle_dfi_sel_8851b(struct mac_ax_adapter *adapter,
...
u32 dle_dfi_ctrl_8851b(struct mac_ax_adapter *adapter, struct dle_dfi_ctrl_t *ctrl_p)
{
...
u32 dle_is_txq_empty_8851b(struct mac_ax_adapter *adapter, u8 *val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, rval32;
...
u32 dle_is_rxq_empty_8851b(struct mac_ax_adapter *adapter, u8 *val)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32, rval32;
...
u32 mac_is_txq_empty_8851b(struct mac_ax_adapter *adapter,
			   struct mac_ax_tx_queue_empty *val)
{
	...
	qempty.grpsel = WDE_QEMPTY_MGQ_SEL_8851B;
	ret = dle_dfi_qempty(adapter, &qempty);
	if (ret != MACSUCCESS) {
...
u32 mac_is_rxq_empty_8851b(struct mac_ax_adapter *adapter,
...
u32 preload_cfg_set_8851b(struct mac_ax_adapter *adapter, enum mac_ax_band band,
			  struct mac_ax_preld_cfg *cfg)
{
	return MACSUCCESS;
}

u32 preload_init_set_8851b(struct mac_ax_adapter *adapter, enum mac_ax_band band,
			   enum mac_ax_qta_mode mode)
{
	return MACSUCCESS;
}

u32 dle_func_en_8851b(struct mac_ax_adapter *adapter, u8 en)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
...
u32 dle_clk_en_8851b(struct mac_ax_adapter *adapter, u8 en)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	...
	if (en == MAC_AX_FUNC_EN)
		val32 |= (B_AX_DLE_WDE_CLK_EN | B_AX_DLE_PLE_CLK_EN | B_AX_AXIDMA_CLK_EN);
	else if (en == MAC_AX_FUNC_DIS)
...
u32 dle_mix_cfg_8851b(struct mac_ax_adapter *adapter, struct dle_mem_t *cfg)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 wde_quota_cfg_8851b(struct mac_ax_adapter *adapter,
...
u32 ple_quota_cfg_8851b(struct mac_ax_adapter *adapter,
...
u32 chk_dle_rdy_8851b(struct mac_ax_adapter *adapter, enum DLE_CTRL_TYPE type)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
