...
static u32 cdma_imr_enable_patch_8851b(struct mac_ax_adapter *adapter, u8 band)
{
...
static u32 phy_intf_imr_enable_patch_8851b(struct mac_ax_adapter *adapter, u8 band)
{
	u32 ret, reg;
	u32 msk = 0;
...
u32 ser_imr_config_patch_8851b(struct mac_ax_adapter *adapter, u8 band,
			       enum mac_ax_hwmod_sel sel)
{
	...
	if (sel == MAC_AX_DMAC_SEL) {
		...
		ret = phy_intf_imr_enable_patch_8851b(adapter, band);
		if (ret) {
			PLTFM_MSG_ERR("[ERR]phy_intf_imr_enable %d\n", ret);
			return ret;
		}

		ret = cdma_imr_enable_patch_8851b(adapter, band);
		if (ret) {
