		    ...
		    struct mac_ax_pcie_cfgspc_param *param)
{
	...
	if (param->write == 1) {
		...
		if (status != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR] write pcie aspm setting fail %d\n", status);
			return status;
		}
	}
	...
	if (param->read == 1) {
		...
		if (status != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR] read pcie aspm setting fail %d\n", status);
			return status;
		}
	}
		       ...
		       struct mac_ax_intf_info *intf_info)
{
	...
	u32 reg = 0, val32, bd_num, bd_addr_h_p2 = 0;
				 ...
				 struct mac_ax_intf_info *intf_info)
{
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852B) ||
		...
		intf_info_def = p_ops->get_pcie_info_def(adapter);
		if (!intf_info_def)
			PLTFM_MSG_ERR("[ERR] get_pcie_info_def fail\n");
...
u32 clr_idx_all_pcie(struct mac_ax_adapter *adapter)
{
	...
	txch_map.ch0 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH0) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch1 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH1) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch2 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH2) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch3 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH3) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch4 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH4) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch5 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH5) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch6 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH6) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch7 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH7) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch8 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_B0MG) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch9 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_B0HI) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch10 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_B1MG) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch11 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_B1HI) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	txch_map.ch12 =
		IS_TXDMA_CH_VALID(MAC_AX_DMA_H2C) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;

	rxch_map.rxq =
		IS_RXDMA_CH_VALID(MAC_AX_RX_CH_RXQ) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
	rxch_map.rpq =
		IS_RXDMA_CH_VALID(MAC_AX_RX_CH_RPQ) ? MAC_AX_PCIE_ENABLE : MAC_AX_PCIE_IGNORE;
...
u32 pcie_pre_init(struct mac_ax_adapter *adapter, void *param)
{
	...
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("[ERR]disable pcie L1.1 fail %d\n", ret);
		return ret;
	}
	...
	ret = get_aspm_cfg_pcie(adapter);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("Get pcie aspm cfg %d\n", ret);
		return ret;
	}
		       ...
		       struct mac_ax_dbcc_hci_ctrl *info)
{
	...
	enum mac_ax_pcie_func_ctrl func_en;
	u8 ch, pause;
	...
	func_en = pause ? MAC_AX_PCIE_DISABLE : MAC_AX_PCIE_ENABLE;
	pause_txmap.ch0 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH0) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch1 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH1) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch2 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH2) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch3 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH3) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch4 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH4) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch5 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH5) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch6 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH6) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch7 = IS_TXDMA_CH_VALID(MAC_AX_DMA_ACH7) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch8 = IS_TXDMA_CH_VALID(MAC_AX_DMA_B0MG) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch9 = IS_TXDMA_CH_VALID(MAC_AX_DMA_B0HI) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch10 = IS_TXDMA_CH_VALID(MAC_AX_DMA_B1MG) ? func_en : MAC_AX_PCIE_IGNORE;
	pause_txmap.ch11 = IS_TXDMA_CH_VALID(MAC_AX_DMA_B1HI) ? func_en : MAC_AX_PCIE_IGNORE;
...
u32 mac_read_pcie_cfg_spc(struct mac_ax_adapter *adapter, u16 addr, u32 *val)
{
	...
	ret = PLTFM_PCIE_CFG_R32((u16)addr, val);
...
u32 get_aspm_cfg_pcie(struct mac_ax_adapter *adapter)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	u32 ret, cfg_reg = 0;
	...
	if ((cfg_reg & PCIE_BIT_STS_ASPM_MSK) == (PCIE_BIT_STS_CLK_PM | PCIE_BIT_STS_L1) ||
		...
		adapter->pcie_info.host_aspm_en = 1;
