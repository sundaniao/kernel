...
static void _send_bt_country_code(struct btc_t *btc)
{
	...
	hal_btc_notify_ps_tdma(btc, true);
...
void _run_coex(struct btc_t *btc, const char *reason)
{
	...
	if (btc->ctrl.always_freerun) {
		...
		_action_wl_only(btc);
		btc->ctrl.igno_bt = true;
	} else if (wl_smap->rf_off || dm->bt_only ||
	           (wl_smap->lps && !bt->link_info.a2dp_desc.exist)) {
		_action_wl_off(btc);
		btc->ctrl.igno_bt = true;
...
void _update_bt_scbd(struct btc_t *btc, bool only_update)
{
	...
	/* if a2dp act change */
	if (bt->link_info.a2dp_desc.exist != bt->link_info.a2dp_desc.exist_last) {
		status_change = true;
		bt_link_change = true;
	}
