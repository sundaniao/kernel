...
const struct fbtc_slot s_def[] = {
	...
	{  5, SLOT_MIX, 0xe5555555}, /* CXST_EBT */
...
static void _set_bt_scan_pri(struct btc_t *btc)
{
	struct btc_dm *dm = &btc->dm;
	struct btc_wl_info *wl = &btc->cx.wl;
	struct btc_wl_role_info *wl_rinfo = &wl->role_info;
	...
	if (btc->ctrl.always_freerun || dm->wl_only || !wl_smap->busy) {
		bt->scan_rx_low_pri = false;
	} else if ((wl->status.val & BTC_WL_LINK_SCAN) || dm->fddt_train_chk) {
		bt->scan_rx_low_pri = true;
	} else
		switch (mode) {
		case BTC_WLINK_2G_STA:
			/* wifi DL/ wifi UL + BT idle + no inq/
			   wifi UL + only BT HID/HFP exist -> BT scan low pri */
			if ((wl_smap->traffic_dir & BIT(TRAFFIC_DL)) ||
			    (b->status.map.connect ?
			    !(b->a2dp_desc.exist || b->pan_desc.exist) :
	...
	_write_scbd(btc, BTC_WSCB_RXSCAN_PRI, !!(bt->scan_rx_low_pri));
}
...
static void _set_trx_nss(struct btc_t *btc)
{
	...
	if (wl_rinfo->link_mode == BTC_WLINK_NOLINK ||
		...
		tx_limit = 1;
		rx_limit = 1;
	} else if (wl_rinfo->p2p_2g && b->profile_cnt.now) {
			tx_limit = 1;
			rx_limit = 1;
	}
...
static void _action_bt_a2dp(struct btc_t *btc)
{
	...
	case BTC_WIDLE:  /* wl-idle + bt-A2DP */
		policy_type = BTC_CXP_PAUTO_TD20B1;
		break;
	}
