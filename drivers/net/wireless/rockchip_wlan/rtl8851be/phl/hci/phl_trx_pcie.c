...
static void
_updt_phl_trx_stat(
	struct phl_info_t *phl_info, struct rtw_hal_com_t *hal_com)
{
	struct rtw_trx_stat *rx_stat = NULL;
	...
	rx_stat = &hal_com->trx_stat;
	phl_com->phl_stats.rx_err_cnt += rx_stat->rx_err_cnt;
	phl_com->phl_stats.rx_ok_cnt += rx_stat->rx_ok_cnt;
...
static enum rtw_phl_status phl_rx_pcie(struct phl_info_t *phl_info)
{
	...
	for (ch = 0; ch < hci_info->total_rxch_num; ch++) {
		...
		_updt_phl_trx_stat(phl_info, hal_com);
