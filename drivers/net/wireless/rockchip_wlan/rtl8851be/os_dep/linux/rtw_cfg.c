...
static uint rtw_max_xmitbuf_sz = MAX_XMITBUF_SZ;
module_param(rtw_max_xmitbuf_sz, uint, 0644);

static uint rtw_nr_xmitbuf = NR_XMITBUFF;
module_param(rtw_nr_xmitbuf, uint, 0644);

static uint rtw_max_mgnt_xmitbuf_sz = MAX_MGNT_XMITBUF_SZ;
module_param(rtw_max_mgnt_xmitbuf_sz, uint, 0644);

static uint rtw_nr_mgnt_xmitbuf = NR_MGNT_XMITBUFF;
module_param(rtw_nr_mgnt_xmitbuf, uint, 0644);

static uint rtw_max_recvbuf_sz = MAX_RECVBUF_SZ;
module_param(rtw_max_recvbuf_sz, uint, 0644);

static uint rtw_nr_recvbuf = NR_RECVBUFF;
module_param(rtw_nr_recvbuf, uint, 0644);
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	phl_com->bus_sw_cap.tx_buf_size = rtw_max_xmitbuf_sz;
	phl_com->bus_sw_cap.tx_buf_num = rtw_nr_xmitbuf;
	phl_com->bus_sw_cap.tx_mgnt_buf_size = rtw_max_mgnt_xmitbuf_sz;
	phl_com->bus_sw_cap.tx_mgnt_buf_num = rtw_nr_mgnt_xmitbuf;
	phl_com->bus_sw_cap.rx_buf_size = rtw_max_recvbuf_sz;
	phl_com->bus_sw_cap.rx_buf_num = rtw_nr_recvbuf;
#endif /* defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI) */
